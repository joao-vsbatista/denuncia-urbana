        ...
        >
          {filtered.length === 0 ? (
            ...
            filtered.map((report) => {
              ...
              return (
                <TouchableOpacity key={report.id} style={styles.card} onPress={() => onSelectReport(report)} activeOpacity={0.75}>
                  ...
                  {report.photo && <Image source={{ uri: report.photo }} style={styles.thumb} />}
                </TouchableOpacity>
              );
            })
          )}
...
const styles = StyleSheet.create({
  ...
  cardDesc: { fontSize: 13, color: COLORS.gray600, lineHeight: 18 },
